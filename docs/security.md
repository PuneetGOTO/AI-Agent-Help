...
| 流量竊聽                   | Kubernetes ingress TLS baseline；public production 強制 secure cookie 與 HTTPS S3 endpoint                                                                  | DB/Redis/provider 全部使用 TLS；Compose loopback MinIO HTTP 不可照搬到 public deployment；內部 mTLS 依合規需要 |
